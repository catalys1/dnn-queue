    ...
    def run(self):
        ...
        while self.running:
            ...
            while msg:
                ...
                jpath = Path(injob['main_file'])
                #joined = os.path.join(jpath,jfile)
                if not self.is_valid_path(str(jpath.parent)):
                    print(f'\r    ERROR:  Path {jpath.parent} is not shared or doesn\'t exist!!')
                    self.job_id -= 1 # redo this number
                elif not jpath.is_file():
                    print(f'\r    ERROR:  File {jpath} doesn\'t exist!!')
                    self.job_id -= 1 # redo this number
                else:
